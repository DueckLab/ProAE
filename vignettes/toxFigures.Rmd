...
The `toxFigures()` function creates publication quality longitudinal bar charts of symptomatic AE profiles. Frequency distributions of PRO-CTCAE scores can be shown over the course of the trial and can be stratified by treatment arm. An R list object is returned with the available PRO-CTCAE items indexed as elements of the list beside a reference table.

```{r}
library(ProAE)
```

In examples 1 and 2, we will use the provided `ProAE::tox_acute` data frame. This data was simulated to demonstrate a common symptomatic AE profile where the drug group experiences acute toxicity followed by symptom abatement over the course of treatment.
...
6)  modify the x axis label
7)  include a box around the summary measures setting the `summary_highlight` parameter to `TRUE`
8)  adjust the appearance of the x axis label angle using the `x_lab_angle` 
